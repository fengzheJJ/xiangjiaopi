...
- (void)setUpTitleViewsPosition {
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        ...
        TitleAlignmentStyle alignmentStyle = self.segmentStyle.titleAlignmentStyle;
        if (self.segmentStyle.isAutoAdjustTitlesWidth) {
            ...
            if (alignmentStyle == TitleAlignmentStyleCenter) {
                addedMargin = allTitlesWidth < self.scrollView.bounds.size.width ? (self.scrollView.bounds.size.width - allTitlesWidth)/2 : 0 ;
            }
            else if (alignmentStyle == TitleAlignmentStyleAverage)
            {
        ...
        for (ZJTitleView *titleView in self.titleViews) {
            ...
            if (alignmentStyle == TitleAlignmentStyleCenter) {
                titleX = lastLableMaxX + addedMargin;
...
- (void)setupScrollLineAndCover {
    ...
    CGFloat coverX = firstLabel.zj_x + (firstLabel.zj_width - self.segmentStyle.scrollLineWidth)/2 ;
    CGFloat coverW = self.segmentStyle.scrollLineWidth;//firstLabel.zj_width;
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.frame = CGRectMake(coverX , self.zj_height - self.segmentStyle.scrollLineHeight - 7, coverW , self.segmentStyle.scrollLineHeight);
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.scrollLine) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                CGFloat titleWidth = currentTitleView.zj_width;
                CGFloat offxSize = (titleWidth - self.segmentStyle.scrollLineWidth)/2;
                weakSelf.scrollLine.zj_x = currentTitleView.zj_x + offxSize;
                weakSelf.scrollLine.zj_width = self.segmentStyle.scrollLineWidth;
                //currentTitleView.zj_width;
...
//            self.scrollLine.zj_width = oldTitleView.zj_width + wDistance * progress;
            ...
            CGFloat space = CGRectGetMidX(currentTitleView.frame) - CGRectGetMidX(oldTitleView.frame);
            CGFloat fspace = fabs(space);
            float width = fabs(space * progress);
            if (space < 0) {
                CGFloat scrollX = CGRectGetMaxX(oldTitleView.frame) - oldTitleView.zj_width/2 + 10 - width;
                if (scrollX > self.scrollLine.zj_x) {
                    self.scrollLine.zj_x = self.scrollLine.zj_x;
                }
                else
                {
                    self.scrollLine.zj_x = scrollX;
                }
                
                self.scrollLine.zj_width = width < self.segmentStyle.scrollLineWidth ? self.segmentStyle.scrollLineWidth : width;
            }
            else
            {

                CGFloat tempWidth = width + self.segmentStyle.scrollLineWidth;
                if (progress < 1) {
                    if (tempWidth >= fspace) {
                        tempWidth = fspace;
                    }
                    self.scrollLine.zj_width = tempWidth ;
                    self.scrollLine.zj_x = oldTitleView.zj_x + (oldTitleView.zj_width - self.segmentStyle.scrollLineWidth)/2;
                }
  
                
                if (progress >= 1 || oldIndex == currentIndex) {
                
                        [UIView animateWithDuration:0.15 animations:^{
                            self.scrollLine.zj_x = currentTitleView.zj_x + (currentTitleView.zj_width - self.segmentStyle.scrollLineWidth)/2;
                            self.scrollLine.zj_width = self.segmentStyle.scrollLineWidth;
                        } completion:^(BOOL finished) {
                        }];
                }
            }
            self.currentTmpIndex = currentIndex;
...
//            self.scrollLine.zj_x = self.scrollLine.zj_x + (self.scrollLine.zj_width - 10)/2;//oldTitleView.zj_x + xDistance * progress;
//            self.scrollLine.zj_width = 10;
        }
        index++;
    }
//
...
{
    if (!_shadowImage) {
        _shadowImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 50, self.frame.size.width, 100)];
        _shadowImage.backgroundColor = [UIColor redColor];
//        _shadowImage.image = [UIImage imageNamed:@"nav_shadow_image.png"];
    }
    return _shadowImage;
}
