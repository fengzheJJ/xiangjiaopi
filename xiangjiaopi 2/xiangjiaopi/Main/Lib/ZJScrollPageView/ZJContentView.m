...
//    if (page == self.currentIndex && page >= 0) {
...
//    }
    ...
    if (scrollView.contentOffset.x == self.bounds.size.width * self.currentIndex && deltaX > 0) {
        NSInteger currentIndex = (scrollView.contentOffset.x / self.bounds.size.width);
        [self contentViewDidMoveFromIndex:currentIndex toIndex:currentIndex progress:1.0];
        // 调整title
        [self adjustSegmentTitleOffsetToCurrentIndex:currentIndex];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    NSInteger currentIndex = (scrollView.contentOffset.x / self.bounds.size.width);
    [self contentViewDidMoveFromIndex:currentIndex toIndex:currentIndex progress:1.0];
//    [self performSelector:@selector(delayOffset:) withObject:[NSNumber numberWithInt:(int)currentIndex] afterDelay:0.3];
    // 调整title
    [self adjustSegmentTitleOffsetToCurrentIndex:currentIndex];
}

- (void)delayOffset:(NSNumber *)currentIndex
{
    [self contentViewDidMoveFromIndex:[currentIndex integerValue]
                              toIndex:[currentIndex integerValue] progress:1.0];
    [self adjustSegmentTitleOffsetToCurrentIndex:[currentIndex integerValue]];
}
